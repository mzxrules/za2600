...
def dump_mesg_lut():
    ...
    msgTbl = [[], []]
    for i in range(64):
        msgTbl[0].append(f"<(Mesg0+#{i*12})")
        msgTbl[1].append(f">(Mesg0+#{i*12})")
...
for mesg in mesg_data:
    ...
    rawB = [rawPairs[i] for i in range(0, len(rawPairs), 2)]
    rawB = [item for sublist in rawB for item in sublist]
    rawA = [rawPairs[i] for i in range(1, len(rawPairs), 2)]
