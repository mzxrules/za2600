...
def color_to_mzx(data):
    output = f'set "zcol_len" to {len(data)-1}\n'
    for i, v in enumerate(data):
        n = f"zcol{i}"
        r, g, b = v
        output += print_color(f"{n}r", r)
        output += print_color(f"{n}g", g)
        output += print_color(f"{n}b", b)
    return output

output = color_to_mzx(COLOR_TABLE["NTSC"])
output += gen_mzx_atari_color_ids()
with open(f'gen/editor_color_ntsc.txt', "w") as file:
    file.write(output)
